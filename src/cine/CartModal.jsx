...
export default function CartModal({ onClose }) {
  ...
  function handleCartDelete(e,id){
    e.preventDefault();
    const filteredData = cartData.filter((data)=>{
        return id !== data.id;
    })
      ...
      <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-[420px] sm:max-w-[600px] lg:max-w-[790px] p-4 max-h-[90vh] overflow-auto">
        <div className="bg-white shadow-md dark:bg-[#12141D] rounded-2xl overflow-hidden p-5 md:p-9">
          ...
          <div className="space-y-8 lg:space-y-12 max-h-[450px] overflow-auto mb-10 lg:mb-14">
            {cartData.length===0 ?(<p className="text-4xl text-yellow-600">The cart is empty</p>): (
                cartData.map((item) => (
                    <div key={item.id} className="grid grid-cols-[1fr_auto] gap-4">
                      ...
                      <div className="flex justify-between gap-4 items-center">
                        <button onClick={(e)=>handleCartDelete(e,item.id)} className="bg-[#D42967] rounded-md p-2 md:px-4 inline-flex items-center space-x-2 text-white">
